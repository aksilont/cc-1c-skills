# 1C Skills for Gemini CLI (Python)

Автоматическая сборка из [main](https://github.com/aksilont/cc-1c-skills) — навыки 1С:Предприятие 8.3 для AI-агента **Gemini CLI** с рантаймом **Python**.

> Эта ветка генерируется CI на каждый push в main. **Не редактируйте напрямую** — все правки идут в [main](https://github.com/aksilont/cc-1c-skills).

## Установка

1. Скачайте ZIP этой ветки: **Code → Download ZIP** (или `git archive`).
2. Распакуйте в корень своего проекта — должна появиться папка `.gemini/skills/`.
3. Запустите Gemini CLI из этого проекта — навыки станут доступны.

## Требования

- **Python 3.9+**. Установка зависимостей: `pip install -r requirements.txt` (lxml, Pillow, psutil).
- **1С:Предприятие 8.3** — для сборки/разборки EPF/ERF и работы с базами.
- **Node.js 18+** — для `/web-test`.

## Документация

Полные гайды, спецификации и описание навыков — в [main](https://github.com/aksilont/cc-1c-skills).

---

Source: https://github.com/aksilont/cc-1c-skills
Build commit: `fddaca37e7ca1054be980e7438f2f71330540fd3`
